...
import { parseColonCommand } from './command-utils.ts';
import { McpConfigParser, McpClient, McpInstaller } from '../converters/mcp/index.ts';
import { parseEnvInt } from '../../shared/env.js';

const DEFAULT_MCP_CONNECT_TIMEOUT = parseEnvInt(process.env.SYNAPSE_MCP_CONNECT_TIMEOUT, 30000);
...
export class McpCommandHandler {
  ...
  private async callMcpTool(
    ...
    try {
      ...
      const client = new McpClient(serverEntry, { timeout: DEFAULT_MCP_CONNECT_TIMEOUT });
