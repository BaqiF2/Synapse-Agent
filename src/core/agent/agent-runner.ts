...
import { parseEnvInt, parseEnvPositiveInt } from '../../shared/env.ts';
...
/**
 * Todo 状态查询接口 — 最小抽象，避免 core → tools 依赖
 */
export interface ITodoStore {
  get(): { items: Array<{ content: string; status: string }> };
}
...
export interface AgentRunnerOptions {
  ...
  systemPrompt: string;
  toolset: Toolset;
  /** LLM generate 函数（注入，消除对 providers 的静态依赖；不提供时使用延迟动态导入） */
...
export class AgentRunner extends EventEmitter {
  ...
  constructor(options: AgentRunnerOptions) {
    ...
    this.contextOrchestrator = new ContextOrchestrator({ client: options.client, generateFn: options.generateFn, context: options.context });
  ...
  async executeBashCommand(command: string, restart: boolean = false): Promise<string> {
    const bashTool = this.toolset.getTool?.('Bash') as
      | { call(args: unknown): Promise<{ output?: string; message?: string }> }
      | undefined;
  ...
  private async runLoop(userMessage: string, options?: AgentRunOptions): Promise<LoopResult> {
    ...
    for (let i = 0; i < this.maxIterations; i++) {
      ...
      if (result.toolCalls.length === 0) {
        ...
        if (await this.hasIncompleteTodos()) continue;
      ...
      // 处理工具结果（保存历史、检测沙箱权限和连续失败）
      const toolResult = await this.processToolResults(result, signal, consecutiveFailures);
      if (toolResult.loopResult) return toolResult.loopResult;
  ...
  private async processToolResults(
  ...
  ): Promise<{ loopResult?: LoopResult; consecutiveFailures: number }> {
    throwIfAborted(signal);
    ...
    if (perm) {
      ...
      return {
        loopResult: { finalResponse: perm.reason, completedNormally: false, permissionRequest: perm },
        consecutiveFailures: 0,
      };
    }

    // 连续失败检测
    const failures = countConsecutiveFailures(toolResults, previousFailures, this.maxConsecutiveFailures);
    if (failures >= this.maxConsecutiveFailures) {
      const failureMsg = 'Consecutive tool execution failures; stopping.';
      logger.error(failureMsg);
      await this.sm.append(createTextMessage('assistant', failureMsg));
      return { loopResult: { finalResponse: failureMsg, completedNormally: false }, consecutiveFailures: failures };
    }
  ...
  private async hasIncompleteTodos(): Promise<boolean> {
    if (!this.todoStore) return false;
    const incomplete = this.todoStore.get().items.filter((i) => i.status !== 'completed');
    if (incomplete.length === 0) return false;
    const tasks = incomplete.map((i) => `- ${i.content} (${i.status})`).join('\n');
    await this.sm.append(createTextMessage('user',
