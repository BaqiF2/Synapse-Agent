...
/** EnhanceResult 类型定义（从 skills 层提取，避免静态依赖） */
...
export class AutoEnhanceTrigger {
  ...
  private enhancerInstance: unknown = null;
  ...
  /**
   * 延迟加载 SkillEnhancer（避免 core → skills 的静态依赖）
   */
  private async getEnhancer(): Promise<{
    analyzeConversation(conversationPath: string, maxChars: number): unknown;
    shouldEnhance(analysis: unknown): { suggestedAction?: string; existingSkill?: string };
    enhance(analysis: unknown, decision: unknown): EnhanceResult;
  }> {
    if (!this.enhancerInstance) {
      const { SkillEnhancer } = await import('../../skills/generator/skill-enhancer.ts');
      this.enhancerInstance = new SkillEnhancer({
        skillsDir: path.join(this.synapseDir, 'skills'),
        conversationsDir: path.join(this.synapseDir, 'conversations'),
      });
    }
    // SkillEnhancer 实例实现了上述接口方法
    return this.enhancerInstance as Awaited<ReturnType<typeof this.getEnhancer>>;
  }
  ...
  async triggerEnhancement(
    ...
    try {
      const enhancer = await this.getEnhancer();

      // Get max characters from settings
      const maxChars = this.settings.getMaxEnhanceContextChars();

      // Analyze conversation
      const analysis = enhancer.analyzeConversation(conversationPath, maxChars);
