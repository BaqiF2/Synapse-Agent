...
export class FixedBottomRenderer {
  ...
  calculateFixedHeight(taskCount: number): number {
    ...
    return Math.min(visibleTasks + BORDER_LINES + overflowLine, this.config.maxHeight);
  }
  ...
  private renderFallback(): void {
    ...
    if (sortedItems.overflowCount > 0) {
      console.log(`│ ...and ${sortedItems.overflowCount} more │`);
    }
    console.log('└─────────────────────────┘');
  }
  ...
  private renderFixed(): void {
    ...
    // 渲染内容
    const lines = this.buildTaskLines(sortedItems.visible);
    const box = this.buildBox(lines, sortedItems.overflowCount);
   ...
   * 构建带边框的显示框
   */
  private buildBox(lines: string[], overflowCount: number): string {
    const header = 'Tasks';
    const contentLengths = lines.map((line) => this.stripAnsi(line).length);
    const overflowText = overflowCount > 0 ? `...and ${overflowCount} more` : '';
    const overflowLength = overflowText.length;
    ...
    const body = lines.map(
      (line) => `│ ${line}${' '.repeat(innerWidth - this.stripAnsi(line).length)} │`
    );
    if (overflowCount > 0) {
      body.push(`│ ${chalk.gray(overflowText)}${' '.repeat(innerWidth - overflowLength)} │`);
    }
