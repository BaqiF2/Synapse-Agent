 ...
 *
 * @module skill-generation-pipeline
 *
 * Core Exports:
 ...
 *
 * 流程：
 * 1. 从对话历史生成 SkillSpec（通过 LLMProvider）
 * 2. 结构验证
 * 3. 如有错误，通过 LLM 自动修复
 * 4. 重复 2-3 直到通过或达到最大轮次
 */
export class SkillGenerationPipeline {
   ...
   *
   * @param spec - 待验证的技能规格
   * @param provider - LLM Provider（用于修复和语义验证）
   * @param maxRounds - 最大修复轮次
   * @param enableSemantic - 是否启用语义验证
   * @returns 流水线结果
   */
  ...
  private parseFixedSpec(text: string, fallback: SkillSpec): SkillSpec {
    try {
      ...
      return {
        ...
        scripts: fallback.scripts, // 脚本不通过 LLM 修复
      };
    } catch {
