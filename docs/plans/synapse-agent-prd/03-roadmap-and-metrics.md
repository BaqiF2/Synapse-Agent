...
- ✓ Agent Shell Command：实现核心工具（read、write、edit、glob、search、bash、skill:load、command:search、task:*）
- ✓ Extension Shell Command：实现工具转换能力

**2. 工具转换系统**
- ✓ Mcp2Bash 转换器：将 MCP 工具转换为 Bash 命令（启动时解析并缓存）
- ✓ Skill2Bash 转换器：将 Skills 中的脚本转换为 Bash 命令（启动时初始化）
- △ 自动更新组件：监听 SKILLS 目录变化并更新 skill:* 命令（存在但未默认启用）
- ✓ command:search 工具：支持关键词与通配符匹配，发现可用的 mcp:* 和 skill:* 工具
...
**1. 技能搜索 Agent**
- ✓ Skill 子 Agent 实现（`task:skill:search`）
- ✓ 技能索引加载与列表注入（index.json）
- ✓ 基于任务所需功能智能匹配（LLM 语义）
- ✓ 返回 JSON 格式的匹配技能列表
- ✓ 与主 Agent Loop 的集成
- ✓ 技能执行：LLM 读取 SKILL.md 并按指令执行（通过 `skill:load`）

**2. 技能强化 Agent**
- ✓ Skill 子 Agent 实现（`task:skill:enhance`）
- △ 任务执行记录分析（基于 Stop Hook 读取 session）
- △ 决策逻辑：由 LLM/元技能提示词判断 create/enhance/none
- △ 配备元技能：
  - `skill-creator`：指导新技能创建
  - `skill-enhance`：指导技能强化
  - （评估类元技能未内置）
- △ 技能生成和更新（通过 SkillGenerator 写入）
- △ 技能索引维护（依赖索引重建/过期重建）

**3. 元技能系统**
- △ 在 `~/.synapse/skills/` 下安装元技能模板
  - `skill-creator`
  - `skill-enhance`
  - （评估类模板未内置）
- ✓ 元技能与普通技能同级，开发者可自行补充完善

**4. 知识强化机制**
- △ 基于任务执行的自动技能优化（Stop Hook + 子 Agent）
- △ 工具使用模式学习（由 LLM 提示词驱动）
- △ 技能库持续扩展（依赖实际使用与索引重建）
