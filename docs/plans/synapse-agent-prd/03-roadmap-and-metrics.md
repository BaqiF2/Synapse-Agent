...
**2. 工具转换系统**
- ✓ Mcp2Bash 转换器：将 MCP 工具转换为 Bash 命令
- ✓ Fc2Bash 转换器：将 FunctionCalling 工具转换为 Bash 命令
- ✓ Skill2Bash 转换器：将 Skills 中的工具转换为 Bash 命令
- ✓ 工具转 Bash Agent：自动识别工具类型并调用对应转换器
- ✓ 工具索引管理：维护 `~/.synapse/tools/index.json`
...
- 能够将 MCP/FunctionCalling/Skill 工具转换为 Bash 命令
- 能够加载和执行简单的技能
- 文档：架构设计文档、用户使用指南
...
- **工具转换覆盖率**：≥ 95% 的外部工具（MCP、FunctionCalling、Skills）能够成功转换为 Bash 命令
- **命令解析正确率**：≥ 99% 的命令能被正确解析并路由到对应实现
- **自描述完整性**：所有命令都支持 `-h` 和 `--help` 参数
