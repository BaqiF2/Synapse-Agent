...
Skills will be automatically enhanced after task completion.
Note: This will consume additional tokens.
```
...
执行一个多步骤的代码审核任务，包含 5+ 次工具调用：

```
You (3)> 请帮我对 src/agent/agent-runner.ts 进行代码审核：
1. 读取文件内容
2. 检查是否有 TODO 或 FIXME 注释
3. 搜索可能的错误处理问题（如空的 catch 块）
4. 检查函数是否超过 50 行
5. 检查是否有硬编码的魔法值
6. 生成审核报告总结
```
...
**预期行为**:
- Agent 执行多个工具调用（read, search 等）
- 任务完成后显示 `[自动增强检查中...]`
- Agent 分析对话模式，判断这是一个可复用的"代码审核"工作流
- Agent 调用 `skill enhance` 创建或增强技能

**预期输出（示例）**:
```
Agent (3)> [执行代码审核...]
  ✓ read src/agent/agent-runner.ts
  ✓ search "TODO|FIXME" --path src/agent
  ✓ search "catch\\s*\\{\\s*\\}" --path src/agent
  ...
...
[自动增强检查中...]
发现可复用的代码审核模式，正在创建技能...
  ✓ skill enhance --reason "Code review workflow"
已创建技能: code-reviewer
```
...
```bash
# 检查日志中的强化相关记录
grep -i "auto-enhance" ~/.synapse/logs/agent.log | tail -20
```

**预期输出**:
```
[INFO] Auto-enhance triggered, continuing agent loop
```
...
```
You (5)> skill search code review
```
...
```
You (2)> 请按照 skill-creator 技能的指导，帮我规划一个"BUG修改"技能的结构
```
...
- [x] `isRunning()` 方法存在
- [x] `shutdown()` 方法存在
- [x] `isInitialized()` 方法存在
- [x] `getSkillCount()` 方法存在
...
**验证结果**:
- [x] SkillSubAgent 正确创建 AgentRunner
- [x] 使用 `silent` 输出模式
...
**验证结果**:
- [x] 系统提示词包含 4 节结构
- [x] 顺序正确：Role → Tools → Meta Skills → Available Skills
